...
from aux_utils.util import create_model, instantiate_from_config
from aux_datasets.collate_fn import CollateFN
...
def train():
    model_yaml_path = './aux_configs/20250226_baseline.yaml'
    # model
    ...
    print(len(dataloader), gpus_nums, epochs, dataloader_config.train_batch_size)
    # logger
    base_logger, ckpt_logger, image_logger = get_logger(logger_config, model_yaml_path)
    # trainer
    if gpus_nums > 1:
        trainer = pl.Trainer(strategy="ddp", max_epochs=epochs,
                                 logger=base_logger,
                                 callbacks=[image_logger, ckpt_logger],
                                 devices=torch.cuda.device_count(), )

    else:
        trainer = pl.Trainer(max_epochs=epochs, logger=base_logger, callbacks=[image_logger, ckpt_logger])
