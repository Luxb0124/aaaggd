...
class NoiseScheduleVP:
    def __init__(
        ...
        ***
        Update: We support discrete-time diffusion aux_models by implementing a picewise linear interpolation for log_alpha_t.
                We recommend to use schedule='discrete' for the discrete-time diffusion aux_models, especially for high-resolution images.
        ***
...
def model_wrapper(
    ...
    We support four types of the diffusion model by setting `model_type`:
        ...
        3. "v": velocity prediction model. (Trained by predicting the velocity).
            ...
            [1] Salimans, Tim, and Jonathan Ho. "Progressive distillation for fast sampling of diffusion aux_models."
    ...
    We support three types of guided sampling by DPMs by setting `guidance_type`:
        ...
        2. "classifier": classifier guidance sampling [3] by DPMs and another classifier.
            ...
            [3] P. Dhariwal and A. Q. Nichol, "Diffusion aux_models beat GANs on image synthesis,"
    ...
    def noise_pred_fn(x, t_continuous, cond=None):
        t_input = get_model_input_time(t_continuous)
        if cond is None:
            output = model.generate(x, t_input, **model_kwargs)
        else:
            output = model.generate(x, t_input, cond, **model_kwargs)
        if model_type == "noise":
            return output
...
class DPM_Solver:
    def __init__(
        ...
        We also support the "dynamic thresholding" method in Imagen[1]. For pixel-space diffusion aux_models, you
        ...
        [1] Chitwan Saharia, William Chan, Saurabh Saxena, Lala Li, Jay Whang, Emily Denton, Seyed Kamyar Seyed Ghasemipour,
            Burcu Karagol Ayan, S Sara Mahdavi, Rapha Gontijo Lopes, et al. Photorealistic text-to-image diffusion aux_models
    ...
    def dpm_solver_adaptive(self, x, order, t_T, t_0, h_init=0.05, atol=0.0078, rtol=0.05, theta=0.9, t_err=1e-5, solver_type='dpmsolver'):
        ...
        [1] A. Jolicoeur-Martineau, K. Li, R. Piché-Taillefer, T. Kachman, and I. Mitliagkas, "Gotta go fast when generating data with score-based aux_models," arXiv preprint arXiv:2105.14080, 2021.
        """
        ns = self.noise_schedule
    ...
    def sample(self, x, steps=20, t_start=None, t_end=None, order=2, skip_type='time_uniform',
        ...
        Some advices for choosing the algorithm:
            - For **unconditional sampling** or **guided sampling with small guidance scale** by DPMs:
                ...
                e.g., DPM-Solver:
                    >>> df_dpm_solver = DPM_Solver(model_fn, noise_schedule, algorithm_type="dpmsolver")
                    >>> x_sample = df_dpm_solver.sample(x, steps=steps, t_start=t_start, t_end=t_end, order=3,
            ...
            - For **guided sampling with large guidance scale** by DPMs:
                ...
                e.g.
                    >>> df_dpm_solver = DPM_Solver(model_fn, noise_schedule, algorithm_type="dpmsolver++")
                    >>> x_sample = df_dpm_solver.sample(x, steps=steps, t_start=t_start, t_end=t_end, order=2,
        ...
        Args:
            ...
            denoise_to_zero: A `bool`. Whether to denoise to time 0 at the final step.
                ...
                for diffusion aux_models sampling by diffusion SDEs for low-resolutional images
...
def interpolate_fn(x, xp, yp):
    ...
    all_x = torch.cat([x.unsqueeze(2), xp.unsqueeze(0).repeat((N, 1, 1)).contiguous()], dim=2)
