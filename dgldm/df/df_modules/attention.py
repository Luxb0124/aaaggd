...
class OffsetRefStrucInter(nn.Module):
    ...
    def forward(self, res_hidden_states, style_content_hidden_states):
        ...
        style_content_hidden_states = style_content_hidden_states.permute(0, 2, 3, 1).reshape(batch, height*width, s_channel).contiguous()
        ...
        res_hidden_states = res_hidden_states.permute(0, 2, 3, 1).reshape(batch, height*width, c_channel).contiguous()
        ...
        reshape_out = hidden_states.permute(0, 2, 1).reshape(batch, c, height, width).contiguous()
