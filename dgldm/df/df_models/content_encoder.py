...
class MLP(nn.Module):
    ...
    def forward(self, x):
        return self.model(x.view(x.size(0), -1).contiguous())
...
class SN(object):
    ...
    def W_(self):
        W_mat = self.weight.view(self.weight.size(0), -1).contiguous()
        if self.transpose:
            W_mat = W_mat.t().contiguous()
...
class Attention(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        theta = theta.view(-1, self. ch // 8, x.shape[2] * x.shape[3]).contiguous()
        phi = phi.view(-1, self. ch // 8, x.shape[2] * x.shape[3] // 4).contiguous()
        g = g.view(-1, self. ch // 2, x.shape[2] * x.shape[3] // 4).contiguous()
        
        beta = F.softmax(torch.bmm(theta.transpose(1, 2).contiguous(), phi), -1)
        
        o = self.o(torch.bmm(g, beta.transpose(1, 2).contiguous()).view(-1, self.ch // 2, x.shape[2], x.shape[3]).contiguous())
