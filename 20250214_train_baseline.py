...
def train():
    model_yaml_path = './configs/20250214_baseline_v1.0.yaml'
    # model
    ...
    if gpus_nums > 1:
        try:
            ...
            trainer = pl.Trainer(gpus=gpus_nums, strategy="ddp",
                                 accelerator='gpu',
                                 max_steps=optimizer_config.max_train_steps,
                                 logger=base_logger,
        ...
        try:
            trainer = pl.Trainer(gpus=gpus_nums, max_steps=optimizer_config.max_train_steps, logger=base_logger,
                                 callbacks=[image_logger, ckpt_logger])
        except:
...
def test():
    model_yaml_path = 'models/2025-01-06-inv_pgt.yaml'
    # model
    model = create_model(model_yaml_path).cpu()
    # some configs
    dataset_config = model.test_dataset_config
    dataloader_config = model.dataloader_config
    test_config = model.test_config
    # dataloader
    dataloader = get_dataloader(dataset_config=dataset_config, dataloader_config=dataloader_config, is_train=False)
    # tester
    trainer = pl.Trainer(gpus=1)
    # load model
    ckpt_path = get_ckpt_path(test_config)
